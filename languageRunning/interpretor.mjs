...
function setup() {
    ...
    "scripts": {
        "compile" : "node --no-warnings .",
        "start": "concurrently \\"qode dist/index.mjs\\" \\"node dist/bots.mjs\\" \\"node dist/index.mjs\\"",
        "help" : "node --no-warnings . --help",
        "setup" : "node --no-warnings . --setup"
    }
}`);

    fs.writeFileSync('./suzuconfig.json', `{\n "input" : "./src/",\n "output" : "./dist"/,\n "usestrict" : true,\n}`);
    fs.unlinkSync("./languageRunning/setupfile.txt");
    console.log("Done setting up!");
}